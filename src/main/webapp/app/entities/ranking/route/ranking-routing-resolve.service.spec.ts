jest.mock('@angular/router');

import { TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRouteSnapshot, Router } from '@angular/router';
import { of } from 'rxjs';

import { IRanking, Ranking } from '../ranking.model';
import { RankingService } from '../service/ranking.service';

import { RankingRoutingResolveService } from './ranking-routing-resolve.service';

describe('Service Tests', () => {
  describe('Ranking routing resolve service', () => {
    let mockRouter: Router;
    let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
    let routingResolveService: RankingRoutingResolveService;
    let service: RankingService;
    let resultRanking: IRanking | undefined;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        providers: [Router, ActivatedRouteSnapshot],
      });
      mockRouter = TestBed.inject(Router);
      mockActivatedRouteSnapshot = TestBed.inject(ActivatedRouteSnapshot);
      routingResolveService = TestBed.inject(RankingRoutingResolveService);
      service = TestBed.inject(RankingService);
      resultRanking = undefined;
    });

    describe('resolve', () => {
      it('should return IRanking returned by find', () => {
        // GIVEN
        service.find = jest.fn(id => of(new HttpResponse({ body: { id } })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultRanking = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultRanking).toEqual({ id: 123 });
      });

      it('should return new IRanking if id is not provided', () => {
        // GIVEN
        service.find = jest.fn();
        mockActivatedRouteSnapshot.params = {};

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultRanking = result;
        });

        // THEN
        expect(service.find).not.toBeCalled();
        expect(resultRanking).toEqual(new Ranking());
      });

      it('should route to 404 page if data not found in server', () => {
        // GIVEN
        jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse({ body: null as unknown as Ranking })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultRanking = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultRanking).toEqual(undefined);
        expect(mockRouter.navigate).toHaveBeenCalledWith(['404']);
      });
    });
  });
});
